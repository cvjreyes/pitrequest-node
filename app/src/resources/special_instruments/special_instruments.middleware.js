...
let storage = multer.diskStorage({
  destination: async (req, file, cb) => {
    const path_pdf = './app/storage/specials/drawings/' + file.originalname.split('.').slice(0, -1) + ".pdf";
    const path_png = './app/storage/specials/drawings/' + file.originalname.split('.').slice(0, -1) + ".png";


    if (!fs.existsSync(path_pdf) && !fs.existsSync(path_png)) {
        console.log("Se añade")
        await cb(null, './app/storage/specials/drawings')
    }else{
